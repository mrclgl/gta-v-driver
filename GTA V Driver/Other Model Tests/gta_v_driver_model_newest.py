...
def gta_v_driver_model(features, mode):
    ...
    #Pooling Layer #1
    pool1 = tf.layers.max_pooling2d(inputs=conv2, pool_size=[2,2], strides=2, padding='same')
    ...
    tf.summary.image("Conv6_Layer", tf.transpose(tf.expand_dims(conv6[0], 0), [3, 1, 2, 0]), max_outputs=5)
    ...
    tf.summary.image("Conv9_Layer", tf.transpose(tf.expand_dims(conv9[0], 0), [3, 1, 2, 0]), max_outputs=5)
    ...
    tf.summary.image("Conv10_Layer", tf.transpose(tf.expand_dims(conv10[0], 0), [3, 1, 2, 0]), max_outputs=5)
    ...
    conv10_flat = tf.layers.dropout(inputs=conv10_flat, rate=0.3, training=mode == learn.ModeKeys.TRAIN)
    
    #Add additional feature to dense layers (speed)
    conv10_flat = tf.concat([tf.reshape(speed, [-1, 1]), conv10_flat], axis=1)
    
    print(conv10_flat.shape)
    
    dense1 = tf.layers.dense(inputs=conv10_flat, units=4096, activation=tf.nn.relu)
    ...
    dense4 = tf.layers.dense(inputs=dropout3, units=4096, activation=tf.nn.relu)
    dropout4 = tf.layers.dropout(inputs=dense4, rate=0.5, training=mode == learn.ModeKeys.TRAIN)
    
    dense5 = tf.layers.dense(inputs=dropout4, units=4096, activation=tf.nn.relu)
    dropout5 = tf.layers.dropout(inputs=dense5, rate=0.5, training=mode == learn.ModeKeys.TRAIN)
	
    dense6 = tf.layers.dense(inputs=dropout5, units=4096, activation=tf.nn.relu)
    dropout6 = tf.layers.dropout(inputs=dense6, rate=0.5, training=mode == learn.ModeKeys.TRAIN)
    
    #Output
    predictions = tf.layers.dense(inputs=dropout6, units=3, activation=None)
